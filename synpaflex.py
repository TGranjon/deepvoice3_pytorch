...
def build_from_path(in_dir, out_dir, num_workers=1, tqdm=lambda x: x):
    '''Preprocesses the SynPaFlex dataset from a given input path into a given output directory.

      Args:
        in_dir: The directory where you have downloaded the SynPaFlex dataset
    ...
    futures = []
    index = 1
    with open(os.path.join(in_dir, 'synpaflex-metadata-10s.csv'), encoding='utf-8') as f:
        for line in f:
            ...
            futures.append(executor.submit(
                partial(_process_utterance, out_dir, index, wav_path, text, phone)))
            index += 1
